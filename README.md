...
We can improve this reduction by performing searches one level up, so that we can take the max reachable nodes from each child of a node and added them together pairwise. If the total reachable from two children are less than 315, we can conclude that no longest path can pass through this node.